namespace SwissTransport.Core
{
    ...
    public class Transport : ITransport, IDisposable
    {
        ...
        public StationBoardRoot GetStationBoard(string station, string id, DateTime time, int ende)
        {
            ...
            var uri = new Uri($"{WebApiHost}stationboard?station={station}&id={id}&time={time}&limit={ende}");
            return HttpClient.GetObject(uri, JsonConvert.DeserializeObject<StationBoardRoot>);

        }

        public Connections GetConnections(string fromStation, string toStation, DateTime date, DateTime time, int ende)
        {
            ...
            var uri = new Uri($"{WebApiHost}connections?from={fromStation}&to={toStation}&date={date}&time={time}&limit={ende}");
